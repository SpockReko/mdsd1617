...
public class ReceptionistBookingTests {
    ...
    public static void setUpClass() throws Exception {
        ...
        receptionist.getIhotelcustomerprovides().initiateBooking("testFirstName1", "20161216", "20170101",
                "testLastName1");
        //initiate booking number 2 with booking Id 2
        receptionist.getIhotelcustomerprovides().initiateBooking("booking2Name", "20161218", "20170102",
                "booking2lastName");
    ...
    public void testInitiateBookingValidParameters() {
        // fourth booking => id 4
        int expected = receptionist.getIhotelcustomerprovides().initiateBooking("testFirstName4", "20161216",
                "20170104", "testLastName4");
        assertEquals(4, expected);
    }

    @Test
    public void testInitiateBookingInvalidDate() {
        int expected = receptionist.getIhotelcustomerprovides().initiateBooking("testFirstName5", "20161216",
                "20150104", "testLastName5");
    ...
    public void testInitiateBookingInvalidDateFormat() {
        ...
        int expected = receptionist.getIhotelcustomerprovides().initiateBooking("testFirstName4", "20161216",
                "201540109", "testLastName4");
        assertEquals(-1, expected);
    }

    @Test
    public void testAddValidRoomToValidBooking() {
        int bookingId = receptionist.getIhotelcustomerprovides().initiateBooking("name", "20161219", "20161714",
                "lastname");
    ...
    public void testAddValidRoomToInvalidBooking() {
        boolean result = receptionist.getIhotelcustomerprovides().addRoomToBooking(DEFAULT_ROOM_TYPE_NAME,
                INVALID_BOOKING_ID);
...
*/
    ...
    public void testGetFreeRooms() {
        //creates the BIGROOM!
        assertTrue(
                admin.getIadministratorprovides().addRoomType("BigRoom", 8000, 10, "Big Screen TV and A Beer Fridge"));
        assertTrue(admin.getIadministratorprovides().addRoom(666, "BigRoom"));
        // Test if we can get the new created room
        EList<FreeRoomTypesDTO> rooms = receptionist.getIhotelcustomerprovides().getFreeRooms(8, "20161216",
                "20170102");
    ...
    public void testAddRoomTypeToBookingValid() {
        ...
        boolean result = receptionist.getIreceptionistprovides().addRoomTypeToBooking(bookingId, DEFAULT_ROOM_TYPE_NAME,
                1);
        assertTrue(result);
    ...
    public void testAddRoomTypeToBookingInvalidAmount() {
        ...
        assertEquals(b.getRoomReservations().size(), 0);
        assertFalse(
                receptionist.getIreceptionistprovides().addRoomTypeToBooking(bookingId, DEFAULT_ROOM_TYPE_NAME, 21));
        assertFalse(
    ...
    public void testRemoveRoomTypeFromBookingValid() {
        ...
        receptionist.getIreceptionistprovides().addRoomTypeToBooking(bookingId, DEFAULT_ROOM_TYPE_NAME, 1);
        assertEquals(b.getRoomReservations().size(), 1);
        boolean result = receptionist.getIreceptionistprovides().removeRoomTypeFromBooking(bookingId,
                DEFAULT_ROOM_TYPE_NAME, 1);
        assertTrue(result);
        assertEquals(b.getRoomReservations().size(), 0);
        clearBooking(bookingId);
    }
    ...
    public void testRemoveRoomTypeFromBookingInvalidRoomBookingId() {
        int invalidId = -1;
        boolean result = receptionist.getIreceptionistprovides().removeRoomTypeFromBooking(invalidId,
                DEFAULT_ROOM_TYPE_NAME, 1);
        assertFalse(result);
    ...
    public void testListCheckIn() {
        ...
        int bookingId = receptionist.getIhotelcustomerprovides().initiateBooking("Stefan", specificDay, "20600202",
                "Bror");
    ...
    public void testListCheckOut() {
        ...
        int bookingId = receptionist.getIhotelcustomerprovides().initiateBooking("Lage", specificDay, "20600202",
                "Bror");
        receptionist.getIreceptionistprovides().addRoomTypeToBooking(bookingId, "Default", 1);
        receptionist.getIhotelcustomerprovides().confirmBooking(bookingId);
        receptionist.getIreceptionistprovides().checkIn(bookingId, 5); // Check in a room today.
        ...
        assertTrue(bookingId + "", receptionist.getIreceptionistprovides().listCheckouts(specificDay,
                currentDate).size() == listSizeBefore + 1);
    ...
    public void testAddExtraCostToRoom() {
        ...
        assertTrue(" bookingPrice = " + bookingPrice,
                bookingPrice == 14100); //14 days * 1000/night + price of testExtra (100)
    }
